...
alter procedure HistorialCajero
as
begin
	...
	begin
		...
		begin transaction 
		begin try
			if exists(select 1 from RegistroCajero)
			begin
				select fechahora,billetesDe100,billetesDe50,total, accion from RegistroCajero 
			end
			else
			begin
				RAISERROR('No existe registro', 16, 1);
				RETURN;
			end
			commit
			break
